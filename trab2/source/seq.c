...
int main(int argc, char *argv[]) { 
    ...
    //double resultTotal = endTimeSeqTotal.tv_sec - startTimeSeqTotal.tv_sec + (endTimeSeqTotal.tv_usec - startTimeSeqTotal.tv_usec) / 1000000.0;

    printf("smoothing: %lfs\n", result);

    FILE *t;
    t = fopen(tempo,"w");
    fprintf(t,"smoothing: %lfs\n", result);
    fclose(t);
